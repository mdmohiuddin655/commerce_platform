...
bounded wire strings:

| Existing bound | Value |
|---|---|
| `maxIdLength` — every canonical opaque id | 64 |
| `CommandEnvelope.commandType` | 64 |

Introducing a second, differently-justified number would mean two ceilings to
reason about and keep aligned. One is enough.
...
"looks sequential" rejection. Only the ceiling is shared.
...
FND-003D1-FIX-001 records this decision and the bound only. **No proof
mechanism, satisfaction policy, permission, command, event or state was added**,
