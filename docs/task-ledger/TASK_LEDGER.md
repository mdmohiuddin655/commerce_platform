...
| FND-003B3 | ADMIN | Custody, delivery-attempt and return lifecycle, including post-dispatch inventory restoration | FND-003B2 | **PARTIAL** | parent; custody acquisition and the picker→rider handoff delivered by FND-003B3A, and the delivery-attempt + refused-order return lifecycles by **FND-003B3B** (contract 0.10) — which also delivers post-dispatch inventory restoration, gated on shop receipt **and** inspection. **Both are accepted and integrated.** **Still outstanding:** successful delivery (`OrderState.delivered`), customer custody, rider assignment `completed` (**B3-C2**, FUTURE), the failed-attempt retry/return consequence and the via-picker return route — B3B enumerates and refuses the last two rather than guessing. Successful delivery additionally needs the **proof-satisfaction policy**, which is now **DECIDED at the policy level** ([ADR-0012](../decisions/ADR-0012-delivery-proof-satisfaction-policy.md), 2026-09-12) and **not implemented** — no challenge lifecycle, evaluator, command, event or state exists — so `CONSTRAINTS.md` invariant 13 is **not discharged** |
...
| FND-003D | ADMIN | Proof and dispute slice: proof-satisfaction contract and fallback dispute workflow | FND-003B | **PARTIAL** | parent; the mechanism-neutral proof/evidence **reference** boundary delivered by FND-003D1, the trusted immutable proof **assessment result** by FND-003D2A, and the **fallback dispute workflow** by FND-003D2B (**as corrected by FND-003D2B-FIX-001 through FIX-006, accepted and integrated**). **The proof-satisfaction policy itself is now DECIDED** (2026-09-12, [ADR-0012](../decisions/ADR-0012-delivery-proof-satisfaction-policy.md), resolving **O8**) — a server-issued, single-use, resource-bound confirmation challenge, verified server-side and fail-closed — but **that ADR decides policy and implements nothing**, so CONSTRAINTS invariant 13 is **still not discharged** and delivery confirmation may not be coded. **How a dispute resolves** is separately undecided — it needs the remaining **FND-003C** money work (**O6 itself is resolved** by ADR-0011) and a resolution slice; FND-003B3B is **done** and decides no dispute outcome |
...
| FND-003D1-BOOKKEEPING-FIX-003 | ADMIN | Correct the root `permission-matrix.md` ordinal wording, guard against its return, and repair two unreproducible evidence claims in the FND-003D1-BOOKKEEPING-FIX-002 report | FND-003D1-BOOKKEEPING-FIX-002 (`4b22173`; ancestry is answerable from git history, which is the authority) | **DONE** | **Documentation and test only — ZERO production change.** No production Dart, no file under any `lib/`, no ADR, no `AGENTS.md`, no `SHARED_BLUEPRINT.md`/`CONSTRAINTS.md` and no other contract document; `ContractVersion.current` stays **0.11** and `Permission.values` / `permissionMatrix` stay **39 / 39**, probed with `Permission.values.indexOf`, not grepped. Exactly **five** files change. **Discharges three debts, all kept on the record rather than deleted.** **(1) Root ordinal conflation.** `permission-matrix.md`'s vocabulary-history sentence called `agent.return.record_receipt` *"the 39th"*. It is the permission that took the **count** from 38 to 39 at **0.10**, but it sits at **zero-based index 12 — the 13th declared**; the 38th declared is `admin.cash.record_reconciliation` and the 39th is `admin.release.view_health`. It now reads *"which took the total from **38 → 39**"*. The 0.10 attribution, the **39 permissions** total and all historical version information are preserved, and no declaration-order claim replaces the old one. **(2) Unstable line pointer** and **(3) unreproducible row totals**, both in the FIX-002 report — see that task's row for what each said and what replaced it. **New guard.** `permission_matrix_doc_consistency_test.dart` gains a prose-pattern guard: that document's prose — generated table and fenced code excluded — may carry **no declaration ordinal**, and must record a **total-count transition ending at the live `Permission.values.length`**. The expected total is derived, so no current count is hard-coded as the source of truth, nothing depends on a source-file declaration position, and nothing is tied to one Markdown line number. The existing derived count, table-row and version checks are preserved unchanged. **Negative controls, proved on temporary material outside the repository worktree:** the published pre-fix document (blob `c12ba13`) **FAILS** both halves of the guard; the corrected document **PASSES** both; `38 → 39`, `38 to 39` and `38 -> 39` are **not** treated as ordinal errors; `0.10 to 0.11` is not misread as a count transition; and the generated table is not searched for ordinals. **Newly found debt, recorded by this task rather than fixed in it:** correcting the canonical header left the FND-003C1-FIX-002 report's §4 opening block quoting a **superseded** `permission-matrix.md` header, with a correction blockquote that described the root wording as outstanding. Both were accurate as history and neither was altered; editing that report was **outside this task's authorized file list** — it was on the do-not-change list — so the debt was recorded instead of widening scope. **RELABELLING DEBT — DISCHARGED by FND-003D1-BOOKKEEPING-FIX-003-FIX-001** (the discovery record above is kept as history, not deleted): that follow-up time-scoped the annotation's status prose to the task each statement describes, and left the verbatim quotation and the captured probe output **byte-identical** — neither was edited by either task. `./tools/run_checks.sh` **ALL CHECKS PASSED**. **GitHub CI: NOT RUN** — no `.github` directory exists; **O7 remains OUTSTANDING**. Report: [FND-003D1-BOOKKEEPING-FIX-003](FND-003D1-BOOKKEEPING-FIX-003-completion-report.md) |
| FND-003D1-BOOKKEEPING-FIX-003-FIX-001 | ADMIN | Time-scope the FND-003C1-FIX-002 report's root-wording status annotation, and clear the follow-up claims FND-003D1-BOOKKEEPING-FIX-003 left behind | FND-003D1-BOOKKEEPING-FIX-003 (`a011add`; ancestry is answerable from git history, which is the authority) | **DONE** | **Ledger/report prose only — ZERO executable, test and contract change.** No Dart file, no file under any `lib/`, no test, no ADR, no `AGENTS.md`, no `SHARED_BLUEPRINT.md`/`CONSTRAINTS.md` and **no contract document** — `docs/contracts/permission-matrix.md` and `permission_matrix_doc_consistency_test.dart` are both untouched by this follow-up. `ContractVersion.current` stays **0.11** and `Permission.values` / `permissionMatrix` stay **39 / 39**, probed. Exactly **four** documentation files change. **The defect.** FND-003D1-BOOKKEEPING-FIX-003 corrected the canonical permission-matrix wording, which left the older FND-003C1-FIX-002 report's §4 correction blockquote asserting in the **present tense** that the root wording was still outstanding, still unauthorized to change and still awaiting its own task — all three false once the header was fixed. **The fix.** That annotation's status sentence is now explicitly time-scoped: it names **FND-003D1-BOOKKEEPING-FIX-002** as the task during which the root wording remained outstanding and deliberately out of scope, records that **FND-003D1-BOOKKEEPING-FIX-003** later corrected the canonical wording, and states that task and commit history — not the paragraph — is the authority on when each change landed. **Nothing historical was rewritten.** The verbatim quotation of the old header and the captured probe output are **byte-identical**, proved by `shasum -a 256` before and after (`05ef90d7…` and `5b92b3ca…`); the explanation that `agent.return.record_receipt` is **not** the 39th declared value, and the fact that both blocks accurately represent the repository as it stood for that task, are preserved unchanged. The FIX-003 report's *"Outstanding defect"* paragraph and its **What remains** row are likewise re-tensed, so neither carries a stale follow-up claim. **Newly found debt, recorded by this task rather than fixed in it — REPORT-STALENESS DEBT, DISCHARGED by FND-003D1-BOOKKEEPING-FIX-003-FIX-002** (the discovery record is kept as history, not deleted): this task's sweep found that the FND-003C1-FIX-002 report's **§12** and its owner-actions line still described the `delivery-proof-boundary.md` permission-count debt **in the present tense**, although **FND-003D1-BOOKKEEPING-FIX-001** had already corrected that document and guarded it with a derived test. Fixing it was outside this task's narrow authorization — *"only correct the surrounding present-state annotation"* about the root wording — so the debt was recorded instead of widening scope. FND-003D1-BOOKKEEPING-FIX-003-FIX-002 time-scoped §12 and the owner-actions line to the FND-003C1-FIX-002 task, named FND-003D1-BOOKKEEPING-FIX-001 as the discharging task, and left the quoted evidence **byte-identical**. `./tools/run_checks.sh` **ALL CHECKS PASSED**. **GitHub CI: NOT RUN** — no `.github` directory exists; **O7 remains OUTSTANDING**. Report: [FND-003D1-BOOKKEEPING-FIX-003-FIX-001](FND-003D1-BOOKKEEPING-FIX-003-FIX-001-completion-report.md) |
| FND-003D1-BOOKKEEPING-FIX-003-FIX-002 | ADMIN | Final semantic closure sweep: time-scope every mechanically stale present-tense claim left in the FND-003C1-FIX-002 and FND-003D1-BOOKKEEPING-FIX-003-FIX-001 reports | FND-003D1-BOOKKEEPING-FIX-003-FIX-001 (`41d3eda`; ancestry is answerable from git history, which is the authority) | **DONE** | **Report and ledger prose only — ZERO executable, test and contract change.** No Dart file, no file under any `lib/`, no test, no ADR, no `AGENTS.md`, no `SHARED_BLUEPRINT.md`/`CONSTRAINTS.md` and **no contract document**. `ContractVersion.current` stays **0.11** and `Permission.values` / `permissionMatrix` stay **39 / 39**, probed. Exactly **four** documentation files change. **Every current-state sentence in both audited reports was checked against the final candidate tree**, and the **seven** mechanically stale claims found were time-scoped, each naming the task that superseded it. **(1) The known one:** §12's *"`delivery-proof-boundary.md` **still asserts** the old permission count in the present tense"* and the matching owner-actions line — discharged by **FND-003D1-BOOKKEEPING-FIX-001**, which corrected that document and added a derived guard; §12's analysis of the two quoted lines is re-tensed and now records that neither sentence survives in that document. **(2) Six more, found by the sweep rather than handed over:** the guard file described as *"(2 tests)"* in §5 and again in §8 (**FND-003D1-BOOKKEEPING-FIX-003** extended the same file, which now carries **9**); *"The second test guards the other half"*, no longer the second test **in the file** though unchanged and still present; and *"13 existing test files already do it"*, which is **15** today. **Verified accurate and therefore left alone:** the three origin stamps in §3 (0.7 / 0.6 / 0.3, all still present); `delivery-proof-dispute.md:345`'s *"was 38 throughout that slice, and is 39 today"*; the slice-scoped **38** references in `version-history.md`, `delivery-proof-assessment.md` and `delivery-proof-dispute.md`; §10's evidence status (DPD1–DPD12 / DPA1–DPA18 **NOT RUN**, `CONSTRAINTS.md` invariant **13** undischarged, successful delivery still not executable); §11's preserved facts; §13's status table (FND-003C **PARTIAL**, FND-003D **PARTIAL**, FND-004 **TODO**, O7 **outstanding**); and *"GitHub CI: NONE"*. **Nothing historical was rewritten.** All three preserved blocks in that report are **byte-identical** — the `permission-matrix.md` header quotation (`05ef90d7…`), the captured probe output (`5b92b3ca…`) and §12's quoted `delivery-proof-boundary.md` lines (`e07bd751…`) — as is every captured command output, including §9's `+1153` and §7's `+2`. `./tools/run_checks.sh` **ALL CHECKS PASSED**. **GitHub CI: NOT RUN** — no `.github` directory exists; **O7 remains OUTSTANDING**. Report: [FND-003D1-BOOKKEEPING-FIX-003-FIX-002](FND-003D1-BOOKKEEPING-FIX-003-FIX-002-completion-report.md) |
| FND-003D3-PROOF-SATISFACTION-POLICY-001 | ADMIN | Define and record the authoritative delivery proof-satisfaction policy required by `CONSTRAINTS.md` invariant 13 | FND-003D2B, FND-003D1-BOOKKEEPING-FIX-003-FIX-002 (`81c623e`; ancestry is answerable from git history, which is the authority) | **DONE** | **Policy decision only — ZERO executable change.** No Dart file, no file under any `lib/`, no test; `ContractVersion.current` stays **0.11** and `Permission.values` / `permissionMatrix` stay **39 / 39**, probed. Exactly **eight** documentation files change, none of them `SHARED_BLUEPRINT.md` (it tracks no such decision). **Made under the explicit owner delegation in this task's brief**, which authorized ADMIN to resolve the previously undefined proof-satisfaction policy within conservative boundaries without returning options to the owner; the untracked decision is now registered as **O8** and **RESOLVED**. **ADR-0012 ACCEPTED** ([ADR-0012](../decisions/ADR-0012-delivery-proof-satisfaction-policy.md), 2026-09-12): delivery proof satisfaction is a **server-issued, short-lived, single-use confirmation challenge** bound to one order, one delivery attempt, one customer and the currently assigned rider (`riderPrincipalId` / `riderAssignmentId` / `riderAssignmentGeneration`) and the current order, custody and rider-slot revisions, fulfilled by the customer side through **either** `rider.delivery.submit_proof` (customer reads the value to the rider) **or** `customer.delivery.confirm_proof` (authenticated User-app confirmation), and **verified server-side** — only the authorized proof verifier of ADR-0009 may record `DeliveryProofAssessmentVerdict.satisfied`. **Customer participation is MANDATORY and NOT SUFFICIENT**, answering the question ADR-0009 left **POLICY-DEFINED / DEFERRED**; it is **not** a veto that ends the order. **Fail-closed throughout**: missing, expired, replayed, mis-bound, stale-revision or malformed input yields no satisfaction, corruption is never downgraded to a verdict, and **no timeout ripens into delivered**. **Insufficient alone**: rider photograph, GPS, timestamp, unbound signature image, rider statement, cash collection, customer non-response, delivery-attempt completion and custody possession. **Privacy**: the raw challenge value never enters an event, payload, notification, log or audit record, comparison is server-side, and photo/location capture is **not** made mandatory; retention, visibility, deletion and legal hold stay DEFERRED. **Replay/idempotency**: single-use enforced atomically with consumption, one challenge per (order, attempt), regeneration invalidates superseded challenges, regeneration and submission are rate-limited, duplicate submissions are idempotent, and ADR-0009's append-only reassessment is unchanged. **Offline**: local capture is permitted, local satisfaction is not; absence stays `DeliveryProofAssessmentFacts.absent` and **no `pending` verdict was introduced**. **Exception review** for accessibility and impossible-primary-method cases must be separately authorized, reason- and reference-bearing, audited, resource-bound, append-only and distinguishable afterwards — **the workflow and its permission are deliberately NOT created here**. **Boundaries kept separate**: cash collection neither satisfies proof nor is settled by it; a dispute does not imply delivered; no fee, refund, fault, compensation, liability, remittance, settlement, reconciliation or commission rule is decided, and `CONSTRAINTS.md` invariants **7** and **11** stand. **INVARIANT 13 — POLICY DEFINED, NOT DISCHARGED.** Both halves are now defined (the fallback dispute workflow by FND-003D2B at 0.9, the policy by ADR-0012), but **no executable proof satisfaction exists**: no challenge lifecycle, no evaluator, no command, no event, no state and no delivery transaction was added. **Successful delivery remains unavailable** — `OrderState.delivered`, `DeliveryAttemptState.delivered`, `CustodyHolderKind.customer` and rider `AssignmentState.completed` are exactly as unreachable as before, and delivery confirmation still may not be coded. The implementing slice is separately scoped and separately reviewed and **must conform to ADR-0012**; weakening any part of it requires a **superseding ADR**, not a policy-version or configuration change. `AGENTS.md` §11 now lists **ADR-0001 → ADR-0012**, each exactly once. `./tools/run_checks.sh` **ALL CHECKS PASSED**. **GitHub CI: NOT RUN** — no `.github` directory exists; **O7 remains OUTSTANDING**. **REVIEWED AND CORRECTED:** candidate `6c4c25a` was independently reviewed by **FND-003D3-PROOF-SATISFACTION-POLICY-001-FINAL-REVIEW-001**, which returned **FIX REQUIRED** on **five** documentation-accuracy findings — **two MAJOR**: (F1) the exception-review section said an ADMIN review *"may conclude satisfaction"*, contradicting the same ADR's mandatory customer participation and opening a bypass; (F2) the customer was required to read a challenge while the ADR named no permitted retrieval channel and its deferral row implied notification transport would deliver the secret — **and three MINOR**: (F3) route 2's rationale claimed it helped a customer with no phone or no signal, which it cannot; (F4) no explicit fail-closed rule for a missing, unknown, unresolvable or unsupported policy version; (F5) the completion report overclaimed a repository-wide sweep that had missed stale current-state wording in `docs/contracts/README.md`. **All five are documentation and policy-wording accuracy only — no executable contract, code or test was wrong, and none changed.** They are corrected by the follow-up commit **FND-003D3-PROOF-SATISFACTION-POLICY-001-FIX-001**, which amends nothing and leaves `6c4c25a` on the record with its review failure intact. Reports: [FND-003D3-PROOF-SATISFACTION-POLICY-001](FND-003D3-PROOF-SATISFACTION-POLICY-001-completion-report.md) · [FND-003D3-PROOF-SATISFACTION-POLICY-001-FIX-001](FND-003D3-PROOF-SATISFACTION-POLICY-001-FIX-001-completion-report.md) |
| FND-003D3-PROOF-SATISFACTION-POLICY-001-FIX-001 | ADMIN | Close the five ambiguities the independent review found in ADR-0012, and correct the stale proof-policy wording it found in `docs/contracts/README.md` | FND-003D3-PROOF-SATISFACTION-POLICY-001 (`6c4c25a`; ancestry is answerable from git history, which is the authority) | **DONE** | **Policy clarification and documentation only — ZERO executable change.** No Dart file, no file under any `lib/`, no test, no `AGENTS.md`, no `CONSTRAINTS.md`, no `SHARED_BLUEPRINT.md` and **no ADR other than ADR-0012**; `ContractVersion.current` stays **0.11** and `Permission.values` / `permissionMatrix` stay **39 / 39**, probed. `rider.delivery.submit_proof` and `customer.delivery.confirm_proof` are byte-identical and no permission is added or widened. Exactly **eight** documentation files change. **FINAL-REVIEW-001 returned FIX REQUIRED on five findings — F1 and F2 MAJOR, F3–F5 MINOR — all corrected here.** **(F1, MAJOR) Exception review can no longer conclude satisfaction.** ADR-0012's Decision 9 is rewritten: a **customer-side fulfilment is REQUIRED**, and no ADMIN exception review or other administrative act may produce `DeliveryProofAssessmentVerdict.satisfied` where none occurred. An exception review **records, classifies and audits** an unresolved case and may **not** mark proof satisfied, mark an order delivered, waive the requirement, assign customer fault, create a fee, resolve a dispute, or substitute a photograph, GPS fix, signature, rider statement, cash collection or administrator judgement for a customer act. An accessibility-compatible **customer act** may be designed later; any substitution **requires a superseding ADR and contract migration, accepted before implementation**. The withdrawn wording is named in the ADR rather than deleted silently. **(F2, MAJOR) Secure challenge retrieval is now specified as a channel class.** The customer obtains the raw value through an **authenticated, customer-scoped pull/retrieval surface** binding the exact customer and resource — normally a future authenticated first-party customer session. **The rider never retrieves it**; a notification may **signal availability** but its payload never carries the value; events, logs, crash reports and audit records never carry it. The ADR states the required class and **implements none of it**, so **route 1 is not executable until that surface exists**. The deferral row *"Notification transport for delivering a challenge to a customer"* is renamed *"…for **signalling challenge availability**"*, pointing at [ADR-0005](../decisions/ADR-0005-notification-stack-decision-required.md) and recording that a missing transport **never** authorizes insecure secret delivery. **(F3, MINOR) Route 2's rationale is corrected** — it no longer claims to help a customer with no phone, no signal or a dead handset (it cannot: it needs an authenticated session on a connected device). It now cites not reading a value aloud at the door, removing rider transcription error, giving a direct authenticated customer act, and an already-signed-in session. **(F4, MINOR) New Decision 11** makes a **missing, unknown, unresolvable, unsupported or unverifiable policy version** fail closed, with **no default inferred** — no built-in fallback and no silent substitution of the latest or a previous version, because an order keeps the version it was quoted under. Mirrored into Decision 6's fail list, Decision 10's forbidden-configuration list, the supersession rule and all three delivery-proof documents. **(F5, MINOR) The sweep overclaim is corrected transparently** in the original report — narrowed to the files actually swept, with the miss recorded rather than hidden — and `docs/contracts/README.md`'s stale *"policy itself is still undefined"*, *"needs the proof-satisfaction policy, which is undefined"* and roadmap-row wording are corrected to: policy decided by ADR-0012, executable proof satisfaction unimplemented, invariant 13 undischarged, successful delivery unavailable, implementation must conform. **Three** such statements were found, not the two reported. **All four policy documents agree semantically** — ADR-0012 and the boundary, assessment and dispute contracts carry the same exception boundary, the same mandatory-participation rule and the same fail-closed policy-resolution rule. **Nothing became reachable.** No challenge model, evaluator, transition, command, event or permission exists; **invariant 13 remains undischarged**; `OrderState.delivered`, `DeliveryAttemptState.delivered`, `CustodyHolderKind.customer` and rider `AssignmentState.completed` remain **unreachable**; **O8 remains RESOLVED at policy level only**. **Out-of-scope staleness found and NOT fixed here:** `docs/contracts/cash-and-payments.md` still lists *"the proof-satisfaction policy"* among things *"still unreachable and untouched"*, and `docs/contracts/version-history.md`'s 0.10 entry and `ADR-0010`'s consequences still say the policy *"is undefined"*. **None of those three files is in this task's authorized list** — `cash-and-payments.md` and `version-history.md` are not listed and every ADR except ADR-0012 is explicitly prohibited — so the finding is **recorded rather than silently widening scope**. **CLASSIFIED AND CLOSED by FND-003D3-PROOF-SATISFACTION-POLICY-001-FIX-002** (the discovery record above is kept as history, not deleted): only **one** of the three was a defect. **(1) `cash-and-payments.md` — genuine current canonical status defect, DISCHARGED by FIX-002**, which replaced the policy in §8's unreachable list with the **executable proof-satisfaction mechanism** and added the explicit policy-versus-mechanism distinction naming ADR-0012, changing no financial, COD, journal, fee, commission, settlement or reconciliation semantics. **(2) `docs/contracts/version-history.md`'s 0.10 entry — NOT debt**: it sits under `## 0.10 — FND-003B3B` in that entry's *"Not added, and not decided"* subsection and states what contract **0.10** did not decide, which was true then and remains true. **No correction required; none made.** **(3) `ADR-0010` — NOT debt**: the *Consequences* of a dated, Accepted decision (2026-09-11, FND-003B3B, contract 0.9 → 0.10) describing the state when that decision was taken. ADRs are dated decision records, not running status pages. **No correction required; none made.** This task's conclusion that all three *"need their own bounded task"* was correct for the first and an over-reading for the other two. `./tools/run_checks.sh` **ALL CHECKS PASSED**. **GitHub CI: NOT RUN** — no `.github` directory exists; **O7 remains OUTSTANDING**. Report: [FND-003D3-PROOF-SATISFACTION-POLICY-001-FIX-001](FND-003D3-PROOF-SATISFACTION-POLICY-001-FIX-001-completion-report.md) |
| FND-003D3-PROOF-SATISFACTION-POLICY-001-FIX-002 | ADMIN | Correct the one current canonical status defect FIX-001 found in `docs/contracts/cash-and-payments.md`, and classify the two dated historical records it had mislabelled as open debt | FND-003D3-PROOF-SATISFACTION-POLICY-001-FIX-001 (`f946c97`; ancestry is answerable from git history, which is the authority) | **DONE** | **Documentation status accuracy only — ZERO executable change.** No Dart file, no file under any `lib/`, no test, no ADR (**including ADR-0010 and ADR-0012**), no `version-history.md`, no `README.md`, no delivery-proof document, no `AGENTS.md`, no `CONSTRAINTS.md` and no `SHARED_BLUEPRINT.md`; `ContractVersion.current` stays **0.11** and `Permission.values` / `permissionMatrix` stay **39 / 39**, probed. Exactly **four** documentation files change. **The one genuine defect.** `docs/contracts/cash-and-payments.md` §8 listed *"the proof-satisfaction policy"* among things *"Still unreachable and untouched"* — a **present-tense current-state claim in a canonical 0.11 contract document**, false since ADR-0012. The list entry now reads *"the **executable proof-satisfaction mechanism**"*, followed by an explicit paragraph: **the proof-satisfaction *policy* is decided** ([ADR-0012](../decisions/ADR-0012-delivery-proof-satisfaction-policy.md), 2026-09-12, **O8**) **and its *mechanism* is not** — ADR-0012 adds no challenge lifecycle, evaluator, command, event, state or permission, so the executable mechanism and the successful-delivery transaction **remain unimplemented and unreachable**, `CONSTRAINTS.md` invariant 13 **remains not discharged**, and **collecting cash still does not satisfy a proof policy**. **No financial, COD, journal, fee, commission, settlement or reconciliation semantics changed**, no executable specification was added, and no unrelated cash/payment documentation was rewritten. **The two non-defects, verified independently and left untouched.** `docs/contracts/version-history.md:553` sits under `## 0.10 — FND-003B3B — delivery attempt and return lifecycles`, inside that entry's *"Not added, and not decided"* subsection: it records what contract **0.10** did not decide, which is accurate and permanent. `ADR-0010:191` is the *Consequences* section of a **dated, Accepted** decision (2026-09-11, FND-003B3B, 0.9 → 0.10) describing the state at that time. **Both are accurate historical snapshots requiring no correction and no follow-up task**, and neither is current debt; the FIX-001 report's conclusion is corrected in place while its discovery evidence and captured output are preserved byte-for-byte. **Nothing became reachable.** `OrderState.delivered`, `DeliveryAttemptState.delivered`, `CustodyHolderKind.customer` and rider `AssignmentState.completed` remain **unreachable**; **invariant 13 remains undischarged at the executable level**; **O8 remains RESOLVED at policy level only**. **No current documentation debt from the FND-003D3 policy slice is carried forward.** `./tools/run_checks.sh` **ALL CHECKS PASSED**. **GitHub CI: NOT RUN** — no `.github` directory exists; **O7 remains OUTSTANDING**. Report: [FND-003D3-PROOF-SATISFACTION-POLICY-001-FIX-002](FND-003D3-PROOF-SATISFACTION-POLICY-001-FIX-002-completion-report.md) |
...
| Slice | Owns | Status |
|---|---|---|
| FND-003B lifecycle | Order, assignment, custody, attempt, return transitions; inventory effects per edge | **PARTIAL** — B1, **all of B2**, **B3A** and **B3B** (attempt + refused-order return) done, **all accepted and integrated**; **successful delivery is still outstanding** — its **policy** is decided ([ADR-0012](../decisions/ADR-0012-delivery-proof-satisfaction-policy.md)) but **unimplemented** — and so are customer custody, rider `completed` (**B3-C2**), the failed-attempt retry/return consequence, the via-picker return route and the post-dispatch cancellation consequence |
...
| FND-003D proof/dispute | Proof-satisfaction policy and fallback dispute workflow | **PARTIAL** — the reference/privacy boundary (FND-003D1), the trusted assessment **result** (FND-003D2A) and the **fallback dispute workflow** (FND-003D2B) are done; the **proof-satisfaction policy itself** is **DECIDED but not implemented** ([ADR-0012](../decisions/ADR-0012-delivery-proof-satisfaction-policy.md), 2026-09-12, **O8** resolved) — the executable challenge lifecycle, evaluator and delivery transaction are still needed before delivery confirmation is coded |
...
| O8 | ~~Decide what a delivery-proof policy actually requires, and whether customer participation is optional, mandatory, sufficient or a veto~~ — **RESOLVED 2026-09-12** under the ADMIN delegation granted in FND-003D3-PROOF-SATISFACTION-POLICY-001, recorded in [ADR-0012](../decisions/ADR-0012-delivery-proof-satisfaction-policy.md). Previously **untracked**: it was carried as *POLICY-DEFINED / DEFERRED* prose in the D1, D2A and D2B documents and had no entry in this table. **Policy only — `CONSTRAINTS.md` invariant 13 is NOT discharged and successful delivery is still not executable.** Weakening any part of the decision needs its **own** superseding ADR and a migration plan, not a policy-version or configuration change. | ~~delivery-confirmation policy~~ — unblocked; the **executable** slice remains outstanding |
